...
WORKSPACE="$(pwd)/vision/models" # workspace for files used in this script
IMG_HEIGHT=640  # output height of images for image resizer
IMG_WIDTH=640   # output width of images for image resizer
...
##### roboflow at home (sorts images for use by AI model)
...
moveto() {
    ...
    if [ $offset -eq 0 ]; then # if offset is 0
        cp      "$WORKSPACE/../datasets/$FOLDER/$c.jpg"            "$WORKSPACE/$FOLDER/yolov5/$type/images" # FIXME: somehow these don't exist
...
cd yolov5 || return
echo "Running model..."
python3 "train.py" --weights "$WEIGHT" --imgsz "640" --cfg "models/yolov5m.yaml" --epochs "$EPOCHS" --data "data/data.yaml"
